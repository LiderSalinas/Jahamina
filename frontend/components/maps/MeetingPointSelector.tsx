...
export function MeetingPointSelector({ reservationId, reservationState = "pendiente", tripState = "programado" }: { reservationId: number; reservationState?: string; tripState?: string }) {
  ...
  const stateLabel = !hasPoint ? "Pendiente" : point?.estado === "confirmado" ? "Confirmado" : point?.estado === "propuesto" ? "Por confirmar" : point?.estado === "rechazado" ? "No acordado" : "Todavía no definido";
  ...
  return <section className={`mobility-panel ${hasPoint ? "has-meeting-point" : "meeting-empty"}`}>
    <div className="meeting-heading"><span className="meeting-icon" aria-hidden>⌖</span><div><p className="eyebrow">Punto de encuentro</p><h2>{hasPoint ? point?.texto : "Punto de encuentro pendiente"}</h2>{!hasPoint && <p className="meeting-empty-copy">Se definirá antes de iniciar el viaje.</p>}</div><span className={point?.estado === "confirmado" ? "badge-active" : "badge-pending"}>{stateLabel}</span></div>
