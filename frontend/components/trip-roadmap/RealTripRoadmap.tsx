...
export function RealTripRoadmap({ reservationId, token, meetingPoint, chat }: { reservationId: number; token: string; meetingPoint: (context: ReservationContext) => ReactNode; chat: (context: ReservationContext) => ReactNode }) {
  ...
  return <article className="reservation-dashboard">
    ...
    <section className="route-area" aria-label="Tu ruta">{stops.length > 0 ? <TripRouteMap stops={stops} realStops={data.paradas} encodedRoute={data.viaje.ruta_codificada} selectedStop={Math.min(selected, stops.length - 1)} onSelect={setSelected} origin={data.viaje.origen} destination={data.viaje.destino} departure={departure.toLocaleTimeString("es-PY", { hour: "2-digit", minute: "2-digit" })}/> : <div className="surface-card route-empty"><p className="ui-eyebrow">Tu ruta</p><h2>{data.viaje.origen} → {data.viaje.destino}</h2><div className="schematic-route" aria-label={`Recorrido desde ${data.viaje.origen} hasta ${data.viaje.destino}`}><span/><div><b>{data.viaje.origen}</b><i/><b>{data.viaje.destino}</b></div><span/></div><div className="route-fallback-meta"><span><small>Salida programada</small><b>{departure.toLocaleTimeString("es-PY", { hour: "2-digit", minute: "2-digit" })}</b></span><p>El trazado detallado aparecerá cuando esté disponible.</p></div></div>}</section>
