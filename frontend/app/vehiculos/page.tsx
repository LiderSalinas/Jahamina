        ...
        <div className="split-layout">
          ...
          <section>
            ...
            {loading ? <AsyncState kind="loading" title="Cargando vehículos…"/> : vehicles.length === 0 ? <AsyncState title="Todavía no registraste vehículos" description="Agregá uno para poder publicar un viaje."/> :
              <div className="vehicle-list">{vehicles.map((vehicle) => <SurfaceCard as="article" className="vehicle-card vehicle-photo-card" key={vehicle.id}>
                <div className="vehicle-card-visual"><VehicleImage brand={vehicle.marca} model={vehicle.modelo} color={vehicle.color} registration={vehicle.matricula} imageUrl={vehicle.imagen_url}/><span className="vehicle-photo-caption">{vehicle.imagen_url ? "Foto del vehículo" : "Vista previa del vehículo"}</span></div>
                <div className="vehicle-copy"><div className="vehicle-heading"><div><p className="eyebrow">Tu vehículo</p><h3>{vehicle.marca} {vehicle.modelo}</h3><p>{vehicle.color} · {vehicle.matricula}</p></div><span className={vehicle.activo ? "badge-active" : "badge-cancelled"}>{vehicle.activo ? "Activo" : "Inactivo"}</span></div><dl className="vehicle-metadata"><div><dt>Capacidad</dt><dd>{vehicle.capacidad} pasajeros</dd></div><div><dt>Estado</dt><dd>{vehicle.activo ? "Listo para publicar" : "No disponible"}</dd></div></dl><ImageUploadControl hasImage={Boolean(vehicle.imagen_url)} uploading={uploadingVehicle === vehicle.id} onUpload={(file) => uploadImage(vehicle, file)} onDelete={() => deleteImage(vehicle)} description="Una foto clara ayuda a que los pasajeros reconozcan tu vehículo."/>{vehicle.activo && <button className="button-secondary vehicle-disable" onClick={() => disable(vehicle.id)} type="button">Desactivar</button>}</div></SurfaceCard>)}</div>}
          </section>
        </div>
      </PageContainer>
    </ProtectedRoute>
